...
`@cavi-ai/api-client` is the **single private API client package** for CAVI Control mobile and portal clients — gateway-agnostic HTTP + WebSocket access to fleet data. Pure TypeScript ESM library; React is an optional peer dep (only the `react/` entry needs it). Ships compiled `dist/`.
