...
4. **`src/providers/{hermes,openclaw,claude,codex,gemini}/`** — provider-specific surfaces over core: gateway providers (Hermes, OpenClaw) ship `client.ts`, `agent-config.ts`, run/SSE providers, `media.ts`, `wiki.ts`, `provider-module.ts`, and thin team-registry wrappers; Claude (Anthropic) is **runtime-only** (`paths.ts` + `provider-module.ts` + a `RuntimeClient` over the Messages API); it also implements the batch surface (`supports.batch`) over Anthropic Message Batches (`submitBatch`/`getBatch`/`cancelBatch`/`getBatchResults`). Plus a `managed-agents/` subtree (beta `managed-agents-2026-04-01`) — `ClaudeManagedAgentClient`, a stateful `RuntimeClient` over the full agent/environment/session lifecycle with steering, outcomes, threads, memory, vaults, session resources, scheduled deployments, webhook verification, and a teams mapper, additively re-exported from the same `providers/claude` entry. Codex (OpenAI) is **runtime-only** — `CodexApiClient` + `createCodexProviderModule` over the OpenAI Responses API; default model `gpt-5-codex`; supports `getRun`/`cancelRun` (background responses) and the batch surface (`supports.batch`) over the OpenAI Batch API. Gemini (Google) is **runtime-only** — `GeminiApiClient` + `createGeminiProviderModule` over the Gemini Developer API (`generativelanguage.googleapis.com`); `x-goog-api-key` auth; model in the URL path (`:generateContent` / `:streamGenerateContent`); explicit model required (no default ships); `getRun`/`cancelRun` throw `EndpointNotFound` (synchronous API).
