...
`0.15.0` release surface.

```text
source: docs/api-client/v0.15.0
publicBasePath: /docs/api-client/v0.15.0
...
The already-published npm package `@cavi-ai/api-client@0.15.0` does not contain
this subsequently generated documentation. Until a future package release
separately publishes it, copy the complete `docs/api-client/v0.15.0` directory
from this repository checkout or its CI documentation artifact to the host's
`/docs/api-client/v0.15.0` public base path. Serve `/docs/api-client` as an
...
The manifest version must equal the documented package version, `0.15.0`.
`manifest.package` must be `@cavi-ai/api-client`, and
`manifest.sourceTarballSha256` must equal
`ccc2e7e99d0ff6f78092abf2b612267e29f4462fa70db905d0799ca45b3d3c0f`, the
SHA-256 digest of the packed `@cavi-ai/api-client@0.15.0` authority artifact.
