...
function deriveRunStatusAt(
  run: Pick<MissionControlRun, "errors" | "updatedAt">,
  now: number,
): MissionControlRunStatus {
  if (run.errors > 0) {
    return "error";
...
export function extractLogIncident(line: string): {
  ...
  const severity: MissionControlIncidentRecord["severity"] = lower.includes(
    "fatal",
  )
    ? "critical"
    : lower.includes("error")
...
export function buildRoutingMatrix(
  ...
  const matrix = new Map<
    string,
    MissionControlRoutingMatrixSnapshot["rows"][number]
  >();
...
export function buildIncidentsSnapshot(
  ...
  const rows = Array.from(incidents.values()).sort(
    (
      left: MissionControlIncidentRecord,
      right: MissionControlIncidentRecord,
    ) => {
  ...
  const blockers = [
    ...rows.filter(
      (incident: MissionControlIncidentRecord) => incident.status === "blocked",
    ),
    ...blockersFromRuns,
  ];
...
export function parseMissionControlRoutingQuery(
