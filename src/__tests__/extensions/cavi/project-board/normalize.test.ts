...
describe("Project Board storage mode", () => {
  it("defaults storage to json-file when the payload omits it", () => {
    const profile = toProjectBoardProfile({ name: "Board", role: "r", emails: [] });
    expect(profile.storage).toBe("json-file");

    const ws = toProjectBoardWorkspaceSnapshot({
      profilePayload: { name: "Board", role: "r", emails: [] },
      sprintPayload: {
        sprint: { id: "current", name: "S", goal: "g", startsOn: null, endsOn: null },
        statusMetrics: { total: 0, todo: 0, inProgress: 0, blocked: 0, done: 0, completionRate: 0 },
      },
      backlogPayload: {
        sections: [],
        priorities: { p0: 0, p1: 0, p2: 0, p3: 0 },
    ...
    expect(ws.sprint.storage).toBe("json-file");
    expect(ws.backlog.storage).toBe("json-file");
  });

  it("honors a sqlite storage mode declared by the backend payload", () => {
    const ws = toProjectBoardWorkspaceSnapshot({
      profilePayload: { name: "Board", role: "r", emails: [], storage: "sqlite" },
      sprintPayload: {
        sprint: { id: "current", name: "S", goal: "g", startsOn: null, endsOn: null },
        statusMetrics: { total: 0, todo: 0, inProgress: 0, blocked: 0, done: 0, completionRate: 0 },
        storage: "sqlite",
    ...
    expect(ws.profile.storage).toBe("sqlite");
    expect(ws.sprint.storage).toBe("sqlite");
    expect(ws.backlog.storage).toBe("sqlite");
  });

  it("falls back to json-file for an unknown storage value", () => {
    const profile = toProjectBoardProfile({
      name: "Board",
      role: "r",
      emails: [],
