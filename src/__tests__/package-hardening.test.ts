...
describe("package hardening", () => {
  ...
  it("composes extension surface paths over the core resolver", () => {
    ...
    expect(resolveWithCavi("gateway.health", "canonical")).toBe(
      resolvePath("gateway.health", "canonical"),
    );
    expect(resolveWithCavi("cavi.operator.snapshot", "canonical")).toBe(
      resolveCaviPath("cavi.operator.snapshot", "canonical"),
    );
    expect(() => resolvePath("cavi.operator.snapshot", "canonical")).toThrow(
      /unknown surface/u,
    );
  });
