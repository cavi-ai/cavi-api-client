...
describe("Deb asset path normalization", () => {
  afterEach(() => {
  ...
  it("maps legacy /cavi-control/deb assets to standalone root paths", async () => {
    ...
    vi.resetModules();
    const { toDebProfile } = await import("../../../cavi/deb/normalize");
...
describe("Deb pod / gateway response shape compatibility", () => {
  ...
  it("accepts sprint updatedAt and optional limitations", () => {
    const ws = toDebWorkspaceSnapshot({
      profilePayload: {
        name: "Deb",
        role: "r",
        emails: [],
  ...
  it("parseDebCallAck tolerates missing limitations on pod ack", () => {
    const traceId = "t1";
    const result = parseDebCallAck(
      {
        ackId: "a1",
        queuedAt: 100,
        queueDepth: 2,
