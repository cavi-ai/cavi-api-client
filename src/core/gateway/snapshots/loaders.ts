  ...
  return {
    loadOverview: async (): Promise<DataEnvelope<GatewayOverviewSnapshot>> =>
      await withOptionalSnapshotFallback({
        area: "overview",
        expectedContract: `WS sessions.list + sessions.usage + ${GATEWAY_SYSTEM_RPC_METHODS.health}/${GATEWAY_SYSTEM_RPC_METHODS.healthSnapshot}`,
    ...
    loadIncidents: async (): Promise<DataEnvelope<GatewayIncidentsSnapshot>> =>
      await withOptionalSnapshotFallback({
        area: "incidents",
        expectedContract: `WS ${GATEWAY_SYSTEM_RPC_METHODS.logsTail} + sessions.list`,
