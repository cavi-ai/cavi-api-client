...
function normalizeGatewayRpcError(
  ...
  fallbackCode: string = ApiClientErrorCode.GatewayError,
): GatewayRpcError {
...
export class GatewayRpcClient {
  ...
  private async requestRaw<TPayload>(
    ...
    if (!ws || ws.readyState !== WebSocket.OPEN) {
      throw new GatewayRpcError(
        ...
        ApiClientErrorCode.SocketUnavailable,
      );
    }
  ...
  private handleMessage(event: MessageEvent): void {
    ...
    if (!frame.ok) {
      ...
      pending.reject(
        new GatewayRpcError(message, frame.error?.code ?? ApiClientErrorCode.RequestFailed),
      );
      return;
    }
