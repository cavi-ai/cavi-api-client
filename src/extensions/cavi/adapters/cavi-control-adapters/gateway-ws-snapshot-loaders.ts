...
export function createGatewayWsSnapshotLoaders(deps: {
  ...
  return {
    ...
    loadAgentRuns: async (filters: AgentRunsFilters) =>
      await coreLoaders.loadAgentRuns(filters) as DataEnvelope<GatewaySessionRunsSnapshot>,
    loadRunDetail: async (key: string) =>
      await coreLoaders.loadRunDetail(key) as DataEnvelope<GatewaySessionRunDetailSnapshot>,
