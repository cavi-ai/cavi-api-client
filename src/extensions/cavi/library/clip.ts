...
export function postLibraryClip<T extends LibraryClipResult = LibraryClipResult>(
  ...
  // Resolve through the canonical surface contract instead of hardcoding the
  // endpoint, matching every other CAVI surface. Defaults to legacy mode so the
  // wire path is unchanged (/library/api/clip); canonical callers pass mode.
  return requestJson<T>(resolveCaviPath("library.clip", "legacy"), {
    method: "POST",
