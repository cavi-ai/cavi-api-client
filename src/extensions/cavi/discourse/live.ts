import type { GatewayWebSocketClient } from "../../../core/ws/index.js";
import type { TaskDiscourseSnapshot } from "../domain/index.js";
import {
  operatorTaskDiscoursePath,
  operatorTaskDiscoursePluginAliasPath,
} from "../contracts/paths.js";
import type { JsonHttpRequest } from "../../../core/http/json-client.js";
import { GATEWAY_RPC_METHODS } from "./contracts.js";
import { normalizeTaskDiscourseSnapshot } from "./normalize.js";

async function requestTaskDiscourseHttp(
  requestJson: JsonHttpRequest,
  taskId: string,
): Promise<unknown> {
  try {
    return await requestJson<unknown>(operatorTaskDiscoursePath(taskId));
  } catch {
    return await requestJson<unknown>(operatorTaskDiscoursePluginAliasPath(taskId));
  }
}

export async function loadTaskDiscourseLive(
  requestJson: JsonHttpRequest,
  wsClient: GatewayWebSocketClient | null | undefined,
  taskId: string,
): Promise<TaskDiscourseSnapshot> {
  const normalizedTaskId = taskId.trim();
  if (!normalizedTaskId) {
    throw new Error("Task id is required.");
  }

  if (wsClient) {
    try {
      const rpcPayload = await wsClient.request<unknown>(
        GATEWAY_RPC_METHODS.discourseTree,
        {
          taskId: normalizedTaskId,
        },
      );
      return normalizeTaskDiscourseSnapshot(rpcPayload, normalizedTaskId);
    } catch {
      // Ignore WS RPC failures and continue with HTTP fallback.
    }
  }

  const httpPayload = await requestTaskDiscourseHttp(requestJson, normalizedTaskId);
  return normalizeTaskDiscourseSnapshot(httpPayload, normalizedTaskId);
}
