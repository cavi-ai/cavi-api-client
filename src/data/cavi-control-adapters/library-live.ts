...
import { requestLibraryApiJson } from "../lib/library-api.js";
import {
  CAVI_TEAM_PORTAL_IDS,
  getPortalTeamIdentity,
  type CaviTeamPortalId,
} from "../lib/canonical-team-registry.js";
import { listPortalLibraryRefs } from "../lib/portal-library-registry.js";

const TEAM_LIBRARY_REFS = listPortalLibraryRefs().filter(
  (ref) => ref.scope === "team" && ref.ownerPortalId,
);

function isCaviTeamPortalId(value: string): value is CaviTeamPortalId {
  return (CAVI_TEAM_PORTAL_IDS as readonly string[]).includes(value);
}

function resolveTeamMeta(ref: (typeof TEAM_LIBRARY_REFS)[number]): {
  name: string;
  lead: string;
} {
  if (ref.ownerPortalId && isCaviTeamPortalId(ref.ownerPortalId)) {
...
function buildTeamStatus(
  ref: (typeof TEAM_LIBRARY_REFS)[number],
...
export async function loadFleetLibraryLive(
  ...
  return {
    ...
    teams: TEAM_LIBRARY_REFS.map((ref) => buildTeamStatus(ref, fleetStatus)),
