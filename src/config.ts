...
export function resolveHttpApiConfigFromEnv(
  ...
  return {
    ...
    hermes: {
      baseUrl: hermesBaseUrl ?? defaults?.hermes?.baseUrl ?? "http://127.0.0.1:8787",
      authToken: cleanToken(hermesAuthToken) ?? defaults?.hermes?.authToken ?? null,
      clientId: hermesClientId ?? defaults?.hermes?.clientId ?? "cavi-api-client",
    },
    library: {
