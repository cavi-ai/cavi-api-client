...
- `RuntimeClient` is the universal contract every provider implements. Gateway backends use `GatewayApiClient`, which implements `RuntimeClient` and adds teams/kanban/workspace/operator surfaces (React’s `GatewayClient*` names are WS-RPC context only). Runtime-only providers (Claude, Codex, Gemini) implement `RuntimeClient` alone.
- `RuntimeClient` includes an optional batch surface (`submitBatch`/`getBatch`/`cancelBatch`/`getBatchResults`), gated by `supports.batch`. Providers that support async batch processing declare this capability; currently implemented by Claude and Codex.
